...
(function() {
  ...
  function populateScenariosList(scenarios) {
    ...
    for (var i = 0; i < scenarios.length; i++) {
      ...
      var link = createElement('a', {'data-scenario-index': $.inArray(thisScenario, twistExecutionDetails.scenarios), href: '#', 'title': thisScenario.name});
      ...
      var passCount = 0;
      for (var i = 0; i < thisScenario.runs.length; i++) {
          var run = thisScenario.runs[i];
          if (run.executionStatus == 'passed') {
              passCount++
          }
      }
      $('h3').html(thisScenario.name);
      var passPercent = Math.floor(passCount / thisScenario.runs.length * 100);
      link.appendChild(createElement('span', {id: 'percent'}, passPercent + '%'));

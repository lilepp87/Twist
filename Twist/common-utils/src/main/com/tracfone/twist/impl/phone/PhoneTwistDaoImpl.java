package com.tracfone.twist.impl.phone;
...
public class PhoneTwistDaoImpl extends AbstractBaseDao implements PhoneTwistDao {
	...
	private final ActiveEsnTestFunctionByBrand activeEsnTestFunctionByBrand;
	...
	private JdbcTemplate brmjdbcTemplate;
	...
	private final UpdateBRMCheck updateBRMCheck;
	...
	public PhoneTwistDaoImpl(JdbcTemplate jdbcTemplate) {
		...
		activeEsnTestFunctionByBrand = new ActiveEsnTestFunctionByBrand();
	    ...
	    updateBRMCheck = new UpdateBRMCheck();
	...
	/*
	 * @Override public synchronized String getNewEsnByPartWithoutAccount(String
	...
	@Override
	public synchronized String getActiveEsnByBrand(String brand) {
		List result = activeEsnTestFunctionByBrand.execute(brand);

		logger.debug("Call to " + GET_TEST_ACTIVE_ESN_BY_BRAND);
		logger.debug("		X_PRODUCT_CODE --> " + brand);
		logger.debug("		result --> " + result);

		if (result.isEmpty()) {
			throw new IllegalArgumentException("No active esns with part number: " + brand);
		} else {
			String esn = (String) result.get(0);
			//resetILDCounter(esn);
			return esn;
	...
	@Override
	public synchronized int finishCdmaByopIgate(String esn, String brand, String status, String isLTE, String isiPhone, String isHd) {
		int result;
		...
		if ("SPRINT".equalsIgnoreCase(brand)) {
			...
			result = finishCdmaByopIgate.update(isLTE, isiPhone, esn, "SPRINT", isActive,isHd);
			TwistUtils.setDelay(1);
			completeCdmaByopIgate.update(esn,"RSS");
		} else {
			logger.info(isiPhone+isLTE);
			result = finishCdmaByopIgate.update(isLTE, isiPhone, esn, "RSS", false, isHd);
			TwistUtils.setDelay(1);
...
//			Assert.assertSame(message, 0, returnCode.intValue());
	...
	private class ActiveEsnTestFunctionByBrand extends MappingSqlQuery {
		public ActiveEsnTestFunctionByBrand() {
			super(getDataSource(), GET_TEST_ACTIVE_ESN_BY_BRAND);
			declareParameter(new SqlParameter("X_PRODUCT_CODE", Types.VARCHAR));
...
//			declareParameter(new SqlOutParameter("objid", Types.VARCHAR));
	...
	private class FinishCdmaByopIgate extends SqlUpdate {
		...
		public int update(String x_pool_name, String x_MPN, String esn, String template, boolean isActive, String isHd) {
			if (isActive) {
				return update(new Object[] {x_pool_name, x_MPN, "ESN_IN_USE: Device is in use", esn, template  });
			} 
			if (isHd.equalsIgnoreCase("Yes")) {
				return update(new Object[] {x_pool_name, x_MPN, "isPIBLock=N,isLostorStolen=N,isBYOPEligible=Y,returnCode=S0048,returnMessage=DISCOUNT_2_ACCESS,validWithCarrier=Y,HDVoice=Y", esn, template });
			}
			else{
				return update(new Object[] {x_pool_name, x_MPN, "isPIBLock=N,isLostorStolen=N,isBYOPEligible=Y,returnCode=S0048,returnMessage=DISCOUNT_2_ACCESS,validWithCarrier=Y,HDVoice=N", esn, template });
			}
	...
	@Override
	public synchronized ArrayList<Object> getRewardBenefitDetails(String transaction_type,String category, String subCategory, String benefitsEarned) {
		ArrayList<Object> benefitDetails = new ArrayList<Object>();
		SqlRowSet  rs = clfyjdbcTemplate.queryForRowSet(GET_REWARD_BENEFIT_DETAILS, new Object[] { transaction_type,category, subCategory, benefitsEarned });
		
		try {
			
			if(category.equalsIgnoreCase("") && subCategory.equalsIgnoreCase("")){
				rs = clfyjdbcTemplate.queryForRowSet(GET_REWARD_BENEFIT_DETAILS_CAT_SUBCAT_NULL, new Object[] { transaction_type, benefitsEarned });
				
			}else if(category.equalsIgnoreCase("")){
				rs = clfyjdbcTemplate.queryForRowSet(GET_REWARD_BENEFIT_DETAILS_CAT_NULL, new Object[] { transaction_type, subCategory, benefitsEarned });
				
			}else if(subCategory.equalsIgnoreCase("")){
				rs = clfyjdbcTemplate.queryForRowSet(GET_REWARD_BENEFIT_DETAILS_SUBCAT_NULL, new Object[] { transaction_type,category,  benefitsEarned });
	...
	@Override
	public synchronized void ChangeDealer(String dealerId, String esn) {
		try {
			this.clfyjdbcTemplate.update(CHANGE_DEALER, new Object[] { dealerId,esn });
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public synchronized int getProgramIdforServicePlan(String sp,boolean isReccuring) {
		...
		try {
			progId = (Integer) clfyjdbcTemplate.queryForObject(GET_SP_PARAM_OBJID_FOR_PINPART, new Object[] { sp,reccuringVal }, Integer.class);
		} catch (Exception e) {
	...
	public String getRecentTicketbyESN(String esn) {
		...
		try {
			...
			caseNumber = (String) clfyjdbcTemplate.queryForObject(GET_RECENT_TICKET_NUMBER, new Object[] { esn,esn},String.class);
	...
	public void updateLatestIGRecordforEsn(String esn) {
		try {
			clfyjdbcTemplate.update(UPDATE_RENET_IG_RECORD, new Object[] { esn });
		} catch (Exception e) {

		}
	}
	
	public String getBusOrgfromBrand(String brand) {

		String busOrg = "";
		try {

			busOrg = (String) clfyjdbcTemplate.queryForObject(GET_BUS_ORG_FROM_BRAND, new Object[] {brand},String.class);

		} catch (Exception e) {
			busOrg = "NO BUS_ORG FOUND";
		}
		return busOrg;
	}
	...
	public String getUPCByPartNumber(String partNumber)  {
		...
		try {
			...
			upcNumber = (String) clfyjdbcTemplate.queryForObject(GET_UPC_FROM_PART_NUMBER, new Object[] { partNumber},String.class);

		} catch (Exception e) {
			upcNumber="noupcfound";
		}
		return upcNumber;
	}
	@Override
	public String getPOSAUnregisteredPin(String brand)  {
		String domain="REDEMPTION CARDS";
		String partSerialNo="";
		String bus_org = getBusOrgfromBrand(brand);
		
		try {

			partSerialNo = (String) clfyjdbcTemplate.queryForObject(GET_POSA_UNREGISTERED_PIN_NUMBER, new Object[] {domain, bus_org},String.class);

		} catch (Exception e) {
			partSerialNo="noSeerialNumberfound";
		//	e.printStackTrace();
		}
		return partSerialNo;
	}
	...
	public String getRedCodefromSnp(String snp)  {
		...
		try {
			...
			redCode = (String) clfyjdbcTemplate.queryForObject(GET_RED_CODE_FROM_SNP, new Object[] {snp},String.class);

		} catch (Exception e) {
			redCode="noSeerialNumberfound";
	...
	public String getSNPfromRedCode(String redCode)  {
		...
		try {
			...
			snp = (String) clfyjdbcTemplate.queryForObject(GET_SMP_FROM_RED_CODE, new Object[] {redCode},String.class);

		} catch (Exception e) {
			snp="noSeerialNumberfound";
	...
	public  void enrollintoAffiliatedPartnerDiscountProgram(String email, String partner) {
		try {
			...
			String pcode= (String) clfyjdbcTemplate.queryForObject(GET_PARTNER_CODE,new Object[] { partner }, String.class);
			
				 this.clfyjdbcTemplate.update(INSERT_INTO_AFFILIATED_PARTNER_DISCOUNT, new Object[] { email,partner,pcode});
	...
	private final static String GET_PARTNER_CODE = "select PARTNER_CODE from SA.TABLE_AFFILIATED_PARTNERS  where PARTNER_NAME= ? and brand ='SIMPLE_MOBILE'";
	
	private final static String INSERT_INTO_AFFILIATED_PARTNER_DISCOUNT ="INSERT INTO SA.TABLE_X_EMPLOYEE_DISCOUNT VALUES (NULL,?,'ACTIVE',sysdate,sysdate+1000,? ,?,'SIMPLE_MOBILE',NULL,NULL,NULL,'AFFILIATED','N') ";
	
	private final static String GET_RED_CODE_FROM_SNP="select x_red_code from TABLE_PART_INST where part_serial_no= ?";
	
	private final static String GET_SMP_FROM_RED_CODE="select  part_serial_no from TABLE_PART_INST where x_red_code= ?";
	
	
	private final static String GET_BUS_ORG_FROM_BRAND="select objid from table_bus_org where org_id= ? ";
	
	private final static String GET_UPC_FROM_PART_NUMBER = "select x_upc from table_part_num where part_number = ? " ;
	
	private final static String GET_POSA_UNREGISTERED_PIN_NUMBER = " SELECT  pc.x_part_serial_no FROM TABLE_X_POSA_CARD_INV pc,table_mod_level ml,table_part_num pn WHERE pn.objid= ml.part_info2part_num  and ml.objid = " +
			" pc.x_posa_inv2part_mod and pc.X_DOMAIN= ? and X_PART_SERIAL_NO in (select pin from sa.tmp_pin)    and pn.part_num2bus_org= ?  AND ROWNUM = 1" ;
	
	private final static String UPDATE_RENET_IG_RECORD = "update Ig_Transaction set status='W' where ACTION_ITEM_ID in (select max(ACTION_ITEM_ID) from  Ig_Transaction where esn=? and status='L')" ;
	...
	private final static String GET_REWARD_BENEFIT_DETAILS="select objid,transaction_type,benefits_earned,category,SUB_CATEGORY,transaction_description from sa.X_REWARD_BENEFIT_EARNING where 1=1 "+
															"  AND TRANSACTION_TYPE= ? AND CATEGORY= ? AND SUB_CATEGORY= ? AND BENEFITS_EARNED= ? and END_DATE > SYSDATE";
	
	
	private final static String GET_REWARD_BENEFIT_DETAILS_CAT_SUBCAT_NULL="select objid,transaction_type,benefits_earned,category,SUB_CATEGORY,transaction_description from sa.X_REWARD_BENEFIT_EARNING where 1=1 "+
			"  AND TRANSACTION_TYPE= ? AND CATEGORY is null AND SUB_CATEGORY is null AND BENEFITS_EARNED= ? and END_DATE > SYSDATE";

	
	private final static String GET_REWARD_BENEFIT_DETAILS_CAT_NULL="select objid,transaction_type,benefits_earned,category,SUB_CATEGORY,transaction_description from sa.X_REWARD_BENEFIT_EARNING where 1=1 "+
			"  AND TRANSACTION_TYPE= ? AND CATEGORY is null  AND SUB_CATEGORY= ? AND BENEFITS_EARNED= ? and END_DATE > SYSDATE";

	
	private final static String GET_REWARD_BENEFIT_DETAILS_SUBCAT_NULL="select objid,transaction_type,benefits_earned,category,SUB_CATEGORY,transaction_description from sa.X_REWARD_BENEFIT_EARNING where 1=1 "+
			"  AND TRANSACTION_TYPE= ? AND CATEGORY= ? AND SUB_CATEGORY is null AND BENEFITS_EARNED= ? and END_DATE > SYSDATE";
	...
	private final static String GET_TEST_ACTIVE_ESN_BY_PART_NUMBER = "SELECT  pi.part_serial_no FROM TABLE_PART_INST PI, TABLE_MOD_LEVEL ML, TABLE_PART_NUM PN, table_part_class pc,"
			...
			+ "and sp.x_min not like 'T%' and sp.part_status='Active' and pi.x_part_inst_status = '52'AND sp.x_expire_dt > SYSDATE and pi.X_DOMAIN||''='PHONES' and pn.part_number = ? and (x_model_number not in ('STAPI4C', 'STAPI4SC', 'STAPI5C') OR "
			+ "pi.x_hex_serial_no is not null) and sp.x_expire_dt > sysdate and ig.ESN=pi.part_serial_no and ig.ESN_TYPE='C' and ROWNUM < 2";

	private final static String GET_TEST_ACTIVE_ESN_BY_BRAND = "SELECT  pi.part_serial_no FROM TABLE_PART_INST PI, TABLE_MOD_LEVEL ML, TABLE_PART_NUM PN, table_part_class pc,"
			+ " TABLE_SITE_PART sp, SA.TEST_IGATE_ESN ig WHERE PI.N_PART_INST2PART_MOD = ML.OBJID AND ML.PART_INFO2PART_NUM = PN.OBJID AND pi.x_part_inst2site_part = sp.objid and pc.objid=pn.part_num2part_class "
			+ "and sp.x_min not like 'T%' and sp.part_status='Active' and pi.x_part_inst_status = '52'AND sp.x_expire_dt > SYSDATE and pi.X_DOMAIN||''='PHONES' and pn.part_number like ? and (x_model_number not in ('STAPI4C', 'STAPI4SC', 'STAPI5C') OR "
			+ "pi.x_hex_serial_no is not null) and sp.x_expire_dt > sysdate and ig.ESN=pi.part_serial_no and ig.ESN_TYPE='C' and ROWNUM < 2";

	
	private final static String GET_PORT_ESN_BY_PART_NUMBER = "SELECT pi.part_serial_no FROM TABLE_PART_INST PI, TABLE_MOD_LEVEL ML, TABLE_PART_NUM PN, table_part_class pc, TABLE_SITE_PART sp, SA.TEST_IGATE_ESN ig, table_part_inst line " +
			"WHERE PI.N_PART_INST2PART_MOD = ML.OBJID AND ML.PART_INFO2PART_NUM = PN.OBJID AND pi.x_part_inst2site_part = sp.objid and pc.objid=pn.part_num2part_class and sp.x_min not like 'T%' and sp.part_status='Active' and " +
			"pi.x_part_inst_status = '52' and pi.X_DOMAIN||''='PHONES' and line.part_to_esn2part_inst = pi.objid AND line.x_domain || '' = 'LINES' and line.part_serial_no not like 'T%' and pn.part_number = ? and ig.ESN=pi.part_serial_no " +
			"and ig.ESN_TYPE='C' and ROWNUM < 2";
	...
	private final static String GET_ACTIVE_ESN_BY_BRAND = "SELECT pi.part_serial_no FROM table_part_inst pi, table_mod_level ml, table_part_num pn, table_bus_org org, table_part_inst line, TABLE_SITE_PART sp "
			...
			+ "sp.x_min not like 'T%' and sp.X_SERVICE_ID = pi.part_serial_no and pi.x_part_inst_status='52' and sp.part_status='Active' and org.s_org_id=? and ROWNUM < 30";
...
//			"and X_DOMAIN||''='PHONES' and x_model_number in ('STAPI4C', 'STAPI4SC', 'STAPI5C') and pi.part_serial_no = ? and pi.x_hex_serial_no is not null and rownum < 2";
	...
	private final static String SQL_MIN_OF_ACTIVE_ESN = "SELECT X_MIN FROM TABLE_SITE_PART WHERE X_SERVICE_ID = ? AND PART_STATUS IN ('Active', 'CarrierPending') order by PART_STATUS";
...
//			(SELECT OBJID FROM table_web_user WHERE LOGIN_NAME = 'cbwuixzz@tracfone.com');
	...
	private final static String GET_LRP_POINTS_BY_EMAIL_FOR_TRANSTYPE = "SELECT distinct AMOUNT FROM X_REWARD_BENEFIT_TRANSACTION where TRANS_TYPE = ? AND WEB_ACCOUNT_ID in (SELECT OBJID FROM table_web_user WHERE LOGIN_NAME = ? )";
	...
	private final static String GET_LRP_POINTS_FOR_PINPART = "SELECT REWARD_POINT FROM MTM_SP_REWARD_PROGRAM WHERE END_DATE >= trunc(sysdate) and SERVICE_PLAN_OBJID IN  (select * from ( SELECT spl.objid "    
			...
			+	"AND pn.part_number = ? and pn.part_num2bus_org = bo.objid order by  objid asc ) where  rownum <2)";
	
	private final static String CHANGE_DEALER="UPDATE TABLE_PART_INST SET part_inst2inv_bin =(SELECT ib.objid FROM table_site s, table_inv_bin ib WHERE s.site_id =? AND ib.bin_name = s.site_id AND rownum < 2) WHERE PART_SERIAL_NO =?";
...
//	private final static String GET_NEW_B2B_ESNS = "SELECT pi.part_serial_no, ts.x_commerce_id, wu.login_name, pi.X_ICCID, wu.login_name as order_id, pi.X_ICCID as password FROM table_x_contact_part_inst cpi, table_web_user wu, x_site_web_accounts swa, table_site ts, table_part_inst pi WHERE 1=1 and pi.x_insert_date > (sysdate-1) AND wu.objid = swa.site_web_acct2web_user AND x_contact_part_inst2contact = wu.web_user2contact and  site_web_acct2site = ts.objid and x_contact_part_inst2part_inst = pi.objid and  ts.x_commerce_id is not null and pi.x_part_inst_status = '50'";
	...
	private final static String GET_ZIPCODE_FOR_ESN="select VAL from table(sa.adfcrm_ret_esn_info( ? )) where COL='ZIPCODE'";
	...
	private final static String GET_RECENT_TICKET_NUMBER="SELECT id_number FROM table_case WHERE x_esn = ? and objid = (select max(objid) from table_case where x_esn=?) ";
	...
	private final static String GET_BAN = "select account_no from pin01.account_t where aac_source = ?";
	
	private final static String GET_CURRENT_OBJID = "select Max(objid) from table_x_call_trans where x_service_id = ? ";
	
	private final static String INSERT_INTO_BRM_CHECK = "Insert into itq_brm_check values(objIDSeq.NEXTVAL,?,?,?,?)";
	
	private final static String GET_ACTION_ITEM_ID_FOR_ESN = "SELECT ACTION_ITEM_ID FROM (select * from ig_transaction where esn= ? ORDER BY ACTION_ITEM_ID DESC) WHERE ROWNUM = 1";

	private final static String GET_MIN_FOR_ESN = "Select MIN FROM (select * from ig_transaction where esn= ? ORDER BY ACTION_ITEM_ID DESC) WHERE ROWNUM = 1";

	public void setOfsjdbcTemplate(JdbcTemplate ofsjdbcTemplate) {
		this.ofsjdbcTemplate = ofsjdbcTemplate;
	}
	
	public void setBrmjdbcTemplate(JdbcTemplate brmjdbcTemplate) {
		this.brmjdbcTemplate = brmjdbcTemplate;
	}
	...
	@Override
	public synchronized String getZipcode(String esn) {
		String Device = (String) this.clfyjdbcTemplate.queryForObject(GET_ZIPCODE_FOR_ESN, new Object[] { esn }, String.class);
		if (Device == null || Device.isEmpty()) {
			throw new IllegalArgumentException("ZipCode not found for : "+ esn);
		} else {
			return Device;
		}
	}
	...
	public String checkpartinststatus(String esn) {
		...
		public String getBANFromWebObj(String webObj) {
			String ban = (String) this.brmjdbcTemplate.queryForObject(GET_BAN, new Object[] { webObj }, String.class);
			if (ban == null || ban.isEmpty()) {
				throw new IllegalArgumentException("BAN not found for : "+ webObj);
			} else {
				return ban;
			}
		}
		...
		public String getCurrentActiveObjID(String esn) {
			String objID = (String) this.clfyjdbcTemplate.queryForObject(GET_CURRENT_OBJID, new Object[] { esn }, String.class);
			if (objID == null || objID.isEmpty()) {
				throw new IllegalArgumentException("ObjID not found for : "+ esn);
			} else {
				return objID;
		...
		public int updateBRMCheck(List<String> insertData){
			return updateBRMCheck.update(insertData.get(0),insertData.get(1),insertData.get(2),Long.parseLong(insertData.get(3)));
		}
		
		private class UpdateBRMCheck extends SqlUpdate {
			public UpdateBRMCheck() {
				super(getDataSource(),INSERT_INTO_BRM_CHECK );
				declareParameter(new SqlParameter("ESN", Types.VARCHAR));
				declareParameter(new SqlParameter("X_MIN", Types.VARCHAR));
				declareParameter(new SqlParameter("BAN", Types.VARCHAR));
				declareParameter(new SqlParameter("X_CT_OBJID", Types.NUMERIC));
				compile();
			}

			public int update(String esn,String x_min,String ban,long x_ct_objid) {
				return update(new Object[] {esn , x_min, ban,x_ct_objid});
			}
		}
		...
		public String getactionitemidbyESN(String esn) {
			String objID = (String) this.clfyjdbcTemplate.queryForObject(GET_ACTION_ITEM_ID_FOR_ESN , new Object[] { esn }, String.class);
			if (objID == null || objID.isEmpty()) {
				throw new IllegalArgumentException("ObjID not found for : "+ esn);
			} else {
				return objID;
		...
		public String getminforESN(String esn) {
			String objID = (String) this.clfyjdbcTemplate.queryForObject(GET_MIN_FOR_ESN , new Object[] { esn }, String.class);
			if (objID == null || objID.isEmpty()) {
				throw new IllegalArgumentException("ObjID not found for : "+ esn);
			} else {
				return objID;

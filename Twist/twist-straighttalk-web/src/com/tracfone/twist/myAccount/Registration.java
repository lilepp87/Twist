...
public class Registration {
	...
	public void registerIphoneModelForCarrier(String model, String simPart, String brand) throws Exception {
		String imei = registeriphone(model, simPart, brand);

		/*
		 * if(simPart.equalsIgnoreCase("TF256PSIMV9N")){
		 * registeriphoneForCdmaRetry(model, esnUtil.getCurrentESN().getSim()
	...
	public String registeriphone(String model, String simPart, String brand) {
		...
		InventoryServices serviceInterface = webServiceImpl.getInventoryServicesSOAP();
		...
		bindingProvider.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, props.getString("inventoryservice_url"));
		...
		/*
		 * if(simPart.equalsIgnoreCase("TF256PSIMV9N")){
		 * Assert.assertEquals(oResp.getResult().getMessage(),
		 * "VD Pending"+imei, oResp.getResult().getMessage());
		 * phoneUtil.finishCdmaByopIgate(imei, "RSS", "NEW", "YES", "YES");
		 * 
		 * }else{ Assert.assertEquals(oResp.getResult().getMessage(), "SUCCESS",
		 * oResp.getResult().getMessage()); }
		 */
		...
		ESN currentEsn = new ESN(phonePartNum, imei);

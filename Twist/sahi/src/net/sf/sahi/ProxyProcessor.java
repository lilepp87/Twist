...
//            System.out.println(uri);
            if (requestFromBrowser.host() != null && requestFromBrowser.host().contains("cloudflare")) {
            	System.out.println(requestFromBrowser.host());
            }
            if (uri != null) {
    ...
    private void processAsProxy(HttpRequest requestFromBrowser) throws Exception {
    	...
    	String url = requestFromBrowser.url().toString();
    	if (url.contains("cloudflare")) {
    		logger.warning("!!! Intial URL: " + url);
    	}
    	Date time = new Date();
...
//          System.out.println("Fetching >> :" + new String(requestFromBrowser.url()));
    ...
    private void processConnect(HttpRequest requestFromBrowser) {
        try {
        	...
        	String domain = requestFromBrowser.host();
//        	Socket socket = client;
        	SSLSocket sslSocket;
        	if (domain.contains("cloudflare")) {
//        		sslSocket = (SSLSocket) (new SSLHelper())
//						.getSSLClientSocketFactory(domain).createSocket(
//								plainSocket,
//								plainSocket.getInetAddress().getHostName(),
//								plainSocket.getPort(), true);
        		sslSocket = new SSLHelper().convertToSecureServerSocket(client,
